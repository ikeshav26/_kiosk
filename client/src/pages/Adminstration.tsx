...
const Administration = () => {
  ...
  const translatedMembers = t('administration.members', { returnObjects: true }) as AdminMember[];
  
  const members = DUMMY_MEMBERS.map((staticMember, index) => {
    const translatedMember = Array.isArray(translatedMembers) ? translatedMembers[index] : null;
    return {
      ...staticMember,
      name: translatedMember?.name || staticMember.name,
      designation: translatedMember?.designation || staticMember.designation,
      deskContent: translatedMember?.deskContent || staticMember.deskContent,
    };
  });
        ...
        {members.length > 0 ? (
          ...
          <div className="h-full flex flex-col items-center justify-center opacity-30 text-slate-400 grayscale py-32">
            ...
            <h3 className="text-4xl font-black uppercase tracking-[0.2em] text-[#002b5c]">
              {t('administration.noRecords')}
            </h3>
            <p className="text-xl mt-4 font-medium">{t('administration.directoryClear')}</p>
          </div>
        )}
      </div>
            ...
            <div className="flex-1 overflow-y-auto w-full custom-scrollbar">
              ...
              <div className="p-6 sm:p-8 md:p-10 bg-white">
                <h2 className="text-2xl sm:text-3xl font-bold text-[#b91c1c] mb-6 tracking-tight">
                  {selectedMember.designation.includes('Chairman') ? t('administration.chairmanDesk') : t('administration.deskLabel', { designation: selectedMember.designation })}
                </h2>
